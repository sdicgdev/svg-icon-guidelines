...
##The Secret Sauce
The secret is embedding ```html<pre><svg></pre>``` tags inline, directly in the HTML with a <use> tag nested inside. This format allows us to select the desired icon sprite from the main spritemap.svg using unique IDs. To add external CSS suport you simply add an "icon-xxxxx" class on each <svg>. You are free to build your styles in SASS if you wish.
